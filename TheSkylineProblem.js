...
//     [12 7]...
// ] is not acceptable;
// the three lines of height 5 should be merged into one in the final output as such: [...[2 3],
//     [4 5],
//     [12 7], ...
// ]
...
var getSkyline = function (buildings) {
    ...
    for (let i = 0; i < buildings.length - 1; i++) {
        ...
        let next = buildings[i + 1];
        if (i === 0) {
            result.push([curr[0], curr[2]])
        }
        if (i + 1 === buildings.length - 1) {
            result.push([curr[1], 0])
        }
        if (curr[2] < next[2]) {
            result.push([next[0], next[2]])
        } else if (curr[2] > next[2]) {
            result.push([curr[1], next[2]])
